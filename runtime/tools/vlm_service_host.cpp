...
void usage(const char * program) {
    std::cerr << "usage: " << program << " --port PORT --model PATH --model-size BYTES --model-sha256 HASH "
              ...
              << "--batch TOKENS --ubatch TOKENS --gpu-layers N\n";
}
}
int main(int argc, char ** argv) {
    std::string port, model, model_size, model_sha256, mmproj, mmproj_size, mmproj_sha256, context, batch, ubatch, gpu_layers;
    for (int index = 1; index < argc; ++index) {
        if (take(index, argc, argv, "--port", &port) || take(index, argc, argv, "--model", &model) ||
            ...
            take(index, argc, argv, "--gpu-layers", &gpu_layers)) continue;
        usage(argv[0]); return 2;
    }
    if (port.empty() || model.empty() || model_size.empty() || model_sha256.empty() || mmproj.empty() || mmproj_size.empty() ||
        mmproj_sha256.empty() || context.empty() || batch.empty() || ubatch.empty() || gpu_layers.empty()) { usage(argv[0]); return 2; }
