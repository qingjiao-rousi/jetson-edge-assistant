...
| P0 | Q4/Q8 各 15 次有冻结摘要，但无公开数值/完整口径 | `docs/jetson-validation.md`；原始 evidence 被忽略 | TTFT/TPS/功耗/内存追问无法公开复核 | 增加协议、采集骨架和空白 CSV；数值待 Jetson | 模板已完成，数据待实测 |
...
| P0 | 当前默认 RAG importer 指向未跟踪文件，且混有 R2.6 实验 | `app/qa/manual_qa.py` 导入 `app/retrieval/active_pipeline.py`；`git status` | 若只提交 tracked diff，发布即缺模块；实验/主线结论混淆 | 原子提交 active pipeline；R2.6 独立实验提交/分支 | 已记录，不擅自拆用户改动 |
...
- 当前评分：**7.8/10**。
- 是否适合作为主项目：**适合端侧 AI 部署/C++ Runtime/Jetson 岗位的主项目，但投递前有两个硬条件**：补一份公开、脱敏、可追溯的 Jetson Q4/Q8 数值表和 2-3 个真实 Demo 截图/GIF；同时整理当前 dirty worktree 为可解释的原子提交。
- 主要加分：C++ Runtime 并非薄命令包装；HTTP/SSE/取消超时、单热 KV 状态、资产合同、RAG/Agent 门禁和测试都有代码证据。
- 主要扣分：公开性能证据缺口、RAG PARTIAL、HTTP 测试本环境 skip、clean-clone+离线包未独立演练。
...
- 建立 Q4_K_M/Q8_0 对照的可复现实验协议与 Jetson 采集骨架，覆盖 TTFT、prefill/decode、tokens/s、板载功耗/资源和稳定性字段；公开数值待实机复核。
...
- 记录 Jetson Release 构建、37/37 CUDA layer offload 与固定单图冒烟的冻结范围；明确视频、多图、高并发、生产鉴权和长稳仍未完成。
...
4. **Q4 为什么优先于 Q8？快多少？** 冻结记录中 Q4/Q8 各有 15 次基线并选择 Q4，但公开仓库没有足够数值可回答具体倍数；应展示按同一协议复核后的表，否则明确“待实测”。
...
1. 公开性能证据：Q4/Q8 TTFT、decode TPS、内存、温度与板载功耗，优先级最高。
2. 真实 Demo：RAG 引用/拒答、单图诊断和 SSE 的 2-3 个短证据，展示价值高于新增 Web 前端。
3. 发布历史：将主线 request-id/active-pipeline 修复与 R2.6 candidate 实验拆成可解释的原子提交。
...
P1：Jetson 上完成 Q4/Q8 复核、单图资源/延迟、30-60 分钟串行 soak、HTTP service 全测试、clean-clone 离线演练；增加最小 systemd unit、日志轮转和退出回收验证。除文档/systemd 草案外均需要 Jetson/资产。

P2：在明确 SLA 后设计鉴权/审计、多 session KV/调度/内存预算、持久状态和故障注入；视频/多图与真实全双工语音必须先定义 API、资源预算、数据和实机验证，不应从当前单图/半双工能力外推。

## 最终证据清单

已具备：92 项模型无关 Python 测试、C++ FakeBackend 测试代码、离线资产合同、固定 submodule、合成手册/图片 fixture、冻结 Jetson 摘要、当前 Q4/MMProj `ready -> terminal -> /quit` 启动复核、公开验证入口、benchmark 协议和 Demo 规范。

待补：Q4/Q8 聚合数值与脱敏原始依据、功耗/RAM/GPU/温度、真实 Jetson CTest 无 skip、RAG 引用/拒答截图、单图 GIF/SSE 记录、clean-clone 离线交付记录、30-60 分钟 soak、systemd 运维证据。生产 SLA、准确率和高并发目前不足以证明。
