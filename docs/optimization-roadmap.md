...
当前阶段：**OPT-1 batch-boundary rollback 已通过 709-token exact-prompt 复验，失效矩阵待完成**
...
| 深入性能优化 | **OPT-1 exact-prompt 已验证，整体仍进行中** | 709-token Q4 clean-commit 30 次配对、输出一致、reviewed 报告 | branch/失效矩阵、稳定性、可选 Q8 对照 |
...
| --- | --- | --- | --- | --- | --- |
| OPT-1 | 真实 `MtmdBackend` Prefix Reuse | **IN_PROGRESS** | Prefill、TTFT、hit tokens | 最高：Runtime 状态管理与正确性 | branch 与失效矩阵 |
...
上述命令只产生 raw evidence；在复制 protocol 字段、检查 cold/hot 输出一致性和缓存失效场景前，结果必须保持 `UNREVIEWED`。709-token exact-prompt 的 correctness 和 Prefill/TTFT 已完成 reviewed 实测；branch、失效恢复、RAM 长稳和更广泛 workload 仍为**待实测**。
