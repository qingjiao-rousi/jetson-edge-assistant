...
from app.retrieval.lexical import ALGORITHM_VERSION,MILESTONE,query_index
...
p=argparse.ArgumentParser();p.add_argument("--phase",choices=("calibration","diagnostic"),required=True);p.add_argument("--database",required=True);p.add_argument("--dataset",required=True);p.add_argument("--output",required=True);p.add_argument("--frozen-calibration");a=p.parse_args();c=load_config(ROOT/"configs/embedding.json");r=calibrate(c,ROOT/a.database,ROOT/a.dataset) if a.phase=="calibration" else diagnostic(c,ROOT/a.database,ROOT/a.dataset,ROOT/a.frozen_calibration);(ROOT/a.output).write_text(json.dumps(r,indent=2,ensure_ascii=False)+"\n");print(json.dumps(r,indent=2,ensure_ascii=False))
