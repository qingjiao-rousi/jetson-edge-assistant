...
from app.retrieval.core import ContractError, EmbeddingSpec, provider_from_config
from app.retrieval.retrieval import MILESTONE, load_config, query_index, sha256_file
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--phase", choices=("calibration", "diagnostic", "holdout"), required=True); parser.add_argument("--config", default="archive/experiments/configs/rag/rag-hybrid-m9.1b-r2.json"); parser.add_argument("--database", required=True); parser.add_argument("--dataset", required=True); parser.add_argument("--frozen-calibration"); parser.add_argument("--output", required=True)
