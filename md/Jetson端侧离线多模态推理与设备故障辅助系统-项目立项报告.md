...
| 项目项 | 立项内容 |
| --- | --- |
| 公司项目周期 | 持续研发中的既有项目，不以本次实习为起止时间 |
| 个人参与周期 | 三个月实习期（12 周个人执行与交付窗口） |
| 项目性质 | 基于公司 `llama.cpp-omni` 的 Jetson Runtime 二次开发与业务验证 |
...
| 交付形态 | 客户演示原型（POC）、性能基线报告、离线部署包、问题清单与技术交接文档 |
| 立项边界 | 个人负责 Jetson Runtime 适配、模型服务、算力优化、可观测性和离线故障排查 POC；音视频采集、VAD/AEC、ASR、TTS、播放及全双工流控由团队其他成员负责 |
| 生产状态 | **不承诺生产上线、最终客户验收或大规模商业化** |

## 1. 项目摘要

公司正在研发面向工业现场运维的端侧实时音视频全双工多模态系统，底层基于开源 `llama.cpp` 扩展形成 `llama.cpp-omni`。公司最终系统包括音视频采集与流控、VAD/AEC、ASR、LLM/VLM、TTS、播放和故障辅助等能力。本项目不是从零实现新的大模型推理框架；个人实习任务是在既有 Runtime 上完成 **Jetson AGX Orin 32GB 的 ARM64/CUDA 适配、C++ Runtime Adapter、模型服务化、算力与 UMA 内存性能评测、离线 RAG/受限工具集成及演示部署**。音视频全双工链路属于公司项目最终功能，但其业务实现不计入个人负责范围。
...
故障描述 / 设备照片 / 告警截图 / 本地维修手册 / 日志
            ...
            原因候选、排查步骤和结构化报告
```
实习期结束时，个人阶段性成果界定为：**可重复演示的客户 POC、目标 Jetson 实测的性能基线报告、ARM64 Docker/systemd 离线部署包，以及问题清单和技术交接文档**。这些成果是公司持续研发项目中的阶段性交付，不等同于整个项目结束、生产级长稳、正式商业化或全零自研推理框架。
...
| 交付状态 | 客户演示 POC、性能基线、部署包和技术交接完成；不表述为生产上线或最终验收 |
...
| 构建修复 | 修复 `mtmd` include 传递依赖（`PRIVATE` 调整为 `PUBLIC`）；需在正式开发中固定 fork/patch 追溯关系 |
...
公司最终系统包含实时音视频全双工交互。麦克风/摄像头采集、VAD/AEC、ASR、视频流输入编排、TTS、播放控制、打断检测和全双工流控由团队其他成员负责。个人子模块提供可被这些模块调用的 Runtime 请求、流式输出、Cancel 和 Context 接口；只有存在实际接口设计、代码提交或联调记录时，才将相应协作计入个人贡献，不能把团队完成的音视频链路写成个人实现。
...
个人 POC 输入：文本 / 图片 / 文档 / 日志 ───────────────────┤
      ...
      ├─ Backend Adapter（CLI/Server/Direct 演进）
...
shutdown()
```
接口是本项目的设计边界，最终签名以固定 Runtime commit 的实际 C++ 实现为准。上层不直接持有上游 CUDA 指针；Backend 可按风险从 `LlamaCliBackend` 原型演进到 `ServerBackend` 或 `DirectBackend`。
...
| W11～W12 | L4T/CUDA/容器兼容矩阵；`nvpmodel` 与运行参数固化；模型只读挂载、Ready 门禁和 active/previous 回退；ARM64 Docker/systemd；冷启动、健康检查、日志、离线安装与异常恢复；演示交接 | 兼容矩阵、推荐配置、两套离线部署脚本、镜像 digest、启动/Ready/恢复记录、客户演示、问题清单和交接文档 | 两套脚本完成基础启停；主路径可断网安装和重启演示；模型损坏/进程退出有可解释结果；不宣称生产级长稳或最终验收 |
...
| 交接材料 | 架构、构建、部署、排障、已知限制、未完成项、后续建议 | Markdown/PDF 文档、问题清单、演示记录 |
...
| 团队音视频接口变更 | Runtime 请求、流式输出或 Cancel 语义无法联调 | 固定请求/响应 Schema 和版本；仅以实际接口评审、代码或联调记录认定个人贡献 |
...
个人核心交付不是声称完成公司的全部音视频系统或生产化大模型平台，而是把既有 Runtime 推进到目标设备，形成有版本、有输入一致性校验、有模型服务边界、有指标、有失败记录、可离线部署、可演示和可交接的工程闭环。个人阶段成功的最终判据是：**客户能够在无外网的 Jetson 环境中按固定流程复现文本/图片故障辅助演示，团队音视频模块能够通过约定接口调用 Runtime，团队能够依据 Manifest、Benchmark 原始数据和部署文档重建与排障，并且所有个人成果均不超出实际证据范围。**
