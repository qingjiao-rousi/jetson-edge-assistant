...
| 文档状态 | 2026-08-04 阶段审查；M8 已冻结，M9 RAG 进行中 |
...
项目在 `llama.cpp-omni`基础上开发，本项目在该基础上负责 Jetson 端侧适配、运行接口封装、模型与性能评测、业务集成和部署验证。当前已完成环境与 CUDA 构建基线、自有 C++ Runtime/HTTP-SSE 服务、Qwen3 Q4/Q8 固定评测、Qwen2.5-VL 单图 Runtime 及应用 API；第 9 周已完成单份合成 Markdown 手册的 FTS5 关键词检索 MVP。语义 RAG 与模型回答闭环、受限工具、Agent、多 session、Docker/systemd 和长稳仍待实现与验收。最新证据化状态统一见 [项目现状总结](./总结.md)。
...
当前 KV Cache 深度主要是 F16/F16 类型、容量、长 context/OOM 和 Prefill/Decode
评测；`DirectBackend` 每次请求 Prefill 前仍清空 KV，服务拒绝非空 `session_id`，所以
没有多轮或 Prefix Cache 命中。M10.1 已登记为待实施任务：先做单热文本会话的 Token
级 LCP 复用、异常失效和命中指标；图片 KV 复用及真正多 session 缓存池不进入第一版。
...
RAG 当前只完成 M9.1A：单份合成 Markdown 手册、按标题切分、真实 tokenizer 计数、SQLite FTS5 关键词检索和稳定 citation。它不是 embedding/向量 RAG，也没有接入 VLM 生成带引用答案。受限文件/故障码/日志工具、Agent、多 session、systemd 和 ARM64 Docker 仍只有需求设计，不能标记为完成。
...
| 第9周 | 本地 RAG | M9.1A 单文档 FTS5 检索与引用 | 进行中；完整 RAG 未完成 |
| 第10周 | KV Prefix 复用、受限工具、Agent、多 session | M10.1 设计及工具/session 范围 | 未开始；无代码与验收证据 |
...
2026-08-04 审查在全新构建目录完成 configure/build，C++ CTest 4/4 通过；Python `unittest` 45/45 通过。服务测试在受限沙盒会明确返回 `BLOCKED_LOOPBACK`，在允许本机回环网络的宿主环境复跑通过。已有 Jetson 文本/VLM 实机证据见 `docs/evaluation/` 和 `benchmark-results/`。这些结果仍不覆盖工具权限、Docker/systemd、并发、故障恢复和长稳验收。
...
- 将当前未提交的 M7～M9.1A 代码、配置、manifest、测试和报告按里程碑归档到 clean commit；
- 统一一键 Runtime 构建脚本，并把模型硬编码门禁收敛为受控 `ModelSpec`/registry；
- 完成多文档/PDF/日志解析、本地 embedding/向量索引、固定检索质量集和失败门禁；
- 将 RAG 上下文与 citation 接入 VLM，输出带可验证来源的 Markdown/JSON 故障报告；
- 按 M10.1 实现文本单热会话 Token 级 KV Prefix 复用、异常失效、命中指标及 Jetson
  冷热对照；
- 实现 2～3 个只读工具的 Schema、权限根目录、超时、错误码和审计日志；
- M10.1 通过后再完成有界 session 隔离、LRU/TTL、请求排队和资源上限；
- 补充真实设备图片/告警/手册案例、人工 rubric、重复 VLM 性能与质量验证；
- 交付至少一种 systemd/ARM64 Docker 部署方式，以及健康检查、异常恢复、回滚和日志记录；
- 完成并发、故障注入、长稳、最终回归、问题清单和技术交接。

## 13. 交付结论

公司 `llama.cpp-omni` 是本项目的基础 Runtime；本项目已经把它推进到 Jetson AGX Orin 离线文本/单图场景，形成可构建、可观测、可评测、可通过 HTTP/SSE 集成的端侧 LLM/VLM 后端。Runtime Adapter、文本服务、量化基线和 VLM 应用单图 API 已有代码、测试和实机证据，可以按限定范围标记为阶段完成；RAG 只完成关键词检索 MVP，工具、Agent、多 session 和部署尚未完成。
