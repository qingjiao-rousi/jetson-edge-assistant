...
| 文档状态 | 2026-08-05 阶段收口；M9.1B-R2.5 保持 PARTIAL，M9.2 与 M10.1 已完成原型闭环 |
...
项目在 `llama.cpp-omni`基础上开发，本项目在该基础上负责 Jetson 端侧适配、运行接口封装、模型与性能评测和原型业务集成。已完成环境与 CUDA 构建基线、自有 C++ Runtime/HTTP-SSE 服务、Qwen3 Q4/Q8 固定评测、Qwen2.5-VL 单图 Runtime 及应用 API、M9.2 本地手册检索/citation/本地模型带来源回答闭环，以及 M10.1 单热文本 KV Prefix 复用。R2.5 的召回指标通过，但无答案拒绝率 `0.50` 未达 `0.75` 门，因此 M9.1B 保持 PARTIAL，holdout 不得重跑或用于调参。受限工具、Agent、多 session、Docker/systemd 和长稳未交付。最新证据化状态统一见 [项目现状总结](./总结.md)。
...
RAG 完成 M9.1A、M9.1B 的 provider 契约、多文档、SQLite 向量/FTS5 hybrid、citation、校准/诊断/一次性最终评测和无命中门禁。R2.5 最终指标为 Recall@1/3 `0.875/0.875`、MRR `0.875`、拒绝率 `0.50`、误命中 `1`；拒答门失败，M9.1B 仍为 PARTIAL。M9.2 已把本地检索和 citation 接入本地模型生成带来源回答。受限工具、Agent、多 session 和部署仍未完成。
...
| 第10周 | KV Prefix 复用 | M10.1 单热文本 session 的实机冷/热/分叉验证 | 已完成；工具、Agent、多 session 未交付 |
...
公司 `llama.cpp-omni` 是本项目的基础 Runtime；本项目已形成可构建、可观测、可评测、可通过 HTTP/SSE 集成的 Jetson 端侧 LLM/VLM 后端原型，并完成 M9.2 本地手册问答闭环和 M10.1 单热文本 KV Prefix 复用。该结论不等同于生产可用、完整工业全双工音视频系统或多用户会话系统；工具、Agent、多 session 和部署未交付。
