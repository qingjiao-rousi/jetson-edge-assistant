...
>
> 项目基于开源 `llama.cpp-omni` 进行二次开发；完整项目最终实现“工业现场实时音视频全双工多模态交互，以及离线设备知识检索与故障辅助”。
>
...
>
> 本文档继续约束既定的 Jetson Runtime、LLM/VLM、量化、RAG/工具和部署主线；音视频采集、VAD/AEC、ASR、TTS、播放及全双工流控由其他模块集成，不改变本文档的任务顺序。

## 1. 业务目标

本仓库为完整工业现场多模态系统提供运行在 Jetson AGX Orin 32GB 上的本地 LLM/VLM Runtime、离线知识检索与故障辅助后端。系统接收音视频模块生成的结构化请求，以及故障描述、设备图片、告警截图、本地维修手册和日志，输出流式文本、带来源引用的原因候选、排查步骤和结构化报告。
...
1. `DirectBackend` 是核心 Runtime：第一版直接链接 `libllama`，实现文本模型单例、Context/KV Cache、Tokenize、Prefill/Decode、Sampling、流式回调、Cancel 和 Reset；VLM 阶段再接入 `libmtmd`。
2. HTTP/JSON/SSE 服务位于 `EdgeOmniRuntime` 之上，不通过 CLI 间接调用模型。
3. `ServerBackend` 仅作为需要进程隔离或兼容既有服务协议时的可选适配器，不是 Direct Runtime 的前置步骤。
4. `llama-cli` 仅用于上游功能基线、参数核验、回归对照和故障诊断，不实现 `LlamaCliBackend` 业务路径。

上层 Request/Response 与 Direct Runtime、可选 Server 适配及后续 VLM 扩展保持解耦。
...
## 21. 当前实现同步（2026-08-06）

本文第 1 至 20 节保留为需求与验收设计，不将计划自动视为已交付。当前实现状态如下：

| 能力 | 当前状态 | 明确边界 |
| --- | --- | --- |
| C++ Runtime、HTTP/JSON/SSE、量化与 VLM | 已完成原型与阶段验证 | 非生产长稳或高并发服务 |
| 手册检索、引用与模型回答 | M9.2 原型闭环完成 | M9.1B-R2.5 质量门仍为 `PARTIAL`，holdout 不得重跑/调参 |
| Agent 与会话 | M10.2 有界进程内 Agent 完成 | 三个只读工具、最多 8 个 session/每个 20 轮；无持久化、鉴权、LRU/TTL 或跨进程共享 |
| 语音 | M11 半双工网关和文本到 TTS 播放完成 | 无外接麦克风实测；无 AEC、打断、流式 TTS 或全双工 |
| 终端 UI | M12 原型完成 | 不是 Web/生产 UI |
| 部署 | 未完成 | 尚无 Docker/systemd、恢复或长稳验收 |

主线代码位于 `runtime/`、`app/`、`configs/`、`knowledge/`、`scripts/` 和 `tests/`；
`scripts/` 只保留薄启动器。`tools/`、`evidence/` 与 `archive/` 分别存放辅助工具、历史证据和归档，
不作为正常演示路径。详细目录职责见 [当前开发指引](../docs/current/development-guide.md)。
