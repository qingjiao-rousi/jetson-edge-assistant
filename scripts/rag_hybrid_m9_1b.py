...
def _load_m9_1a_parser():
    path = ROOT / "scripts" / "build_markdown_rag_index.py"
    module_spec = importlib.util.spec_from_file_location("rag_m9_1a_compat", path)
...
def load_documents(config: dict) -> tuple[list[dict], list[dict]]:
    ...
    for source_spec in config["sources"]:
        ...
        document, parsed_chunks = M9_1A.parse_manual(source)
...
def keyword_ranks(connection: sqlite3.Connection, query: str) -> dict[str, float]:
    terms = M9_1A.re.findall(r"[A-Za-z0-9]+", query.lower())
