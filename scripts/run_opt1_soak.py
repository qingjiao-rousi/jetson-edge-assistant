...
def main():
 ...
 try:
  ...
  if log:log.close()
  raw={"status":"INCOMPLETE" if reason else ("EXPLORATORY_UNREVIEWED" if exploratory else "UNREVIEWED_RAW_RESULT"),"failure_reason":reason,"mode":runtime["prefix_reuse"],"commit":commit,"worktree_clean":clean,"git_status_entries":entries,"git_status_sha256":status_hash,"model_sha256":runtime["model"]["sha256"],"mmproj_sha256":runtime["mmproj"]["sha256"],"config_sha256":hashlib.sha256(pathlib.Path(a.config).read_bytes()).hexdigest(),"runtime_parameters":{key:runtime[key] for key in ("context_tokens","batch_tokens","ubatch_tokens","gpu_layers","host","port")},"prompt_sha256":hashlib.sha256(prompt.encode()).hexdigest(),"clock_locked":locked,"clock_detail":clock_detail,"clock_output":clock_output,"requested_minutes":a.minutes,"started_utc":start_utc if start else None,"ended_utc":end_utc,"measured_duration_seconds":round(end-start,3) if start else 0,"warmup":warmup,"requests":rows,"tegrastats_log":str(tegra_path),"telemetry":telemetry(tegra_path)}
